...
class BakeHouse:
    ...
    def allocate_table(self):
        if(self.__occupied_table_list.get_head() is None):
            self.__occupied_table_list.insert(1,None)
        else:
            ptr=self.__occupied_table_list.get_head()
            temp=None
            for i in range(1,11):
                if(ptr.get_data()!=i):
                    if(temp is None):
                        self.__occupied_table_list.insert(i,temp)
                    else:
                        self.__occupied_table_list.insert(i,temp.get_data())
                    break
                temp=ptr
                if(ptr.get_next()):
                    ptr=ptr.get_next()
